...
module.exports = (client, config) => {
  ...
  fs.readdirSync('./commands/user/').forEach((dir) => {
    ...
    for (let file of UserCommands) {
      ...
      if (pull.name, pull.type == 2) {
        ...
        console.log(`Loaded a file: ${pull.name} (#${client.user_commands.size})`.brightGreen);
        ...
        console.log(`Couldn't load the file ${file}, missing module name value or type isn't 2.`.red)
        continue;
      };
    };
  });
  ...
  if (!config.Client.ID) {
    console.log("Invailed Bot ID?".red + "\n");
    return process.exit();
  };
  ...
  (async () => {
    console.log('Started registering all the application commands.'.yellow);
    ...
    try {
      ...
      console.log('Wtf your code works?'.brightGreen);
    } catch (err) {
      console.log(err);
    }
  })();
